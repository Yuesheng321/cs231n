...
class CaptioningRNN(object):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
        ...
        - wordvec_dim: Dimension W of word vectors.指经过VGG16处理后的维度,提取的特征数
        ...
        # Initialize word vectors 单词由整形转化成向量，可变？？？
    ...
    def loss(self, features, captions):
        ...
        # pass
        # forward propo
        # 将cnn的特征转化成初始隐藏单元h0, shape(N, H)
        h0, cache_h0 = affine_forward(features, W_proj, b_proj)
        
        # 将单词转化成向量x, shape(N, T, W)
        ...
        dW = word_embedding_backward(dx, cache_x)
    ...
    def sample(self, features, max_length=30):
        ...
        At each timestep, we embed嵌入 the current word, pass it and the previous hidden
        ...
        # (1) Embed the previous以前的 word using the learned word embeddings           #
        ...
        for t in range(max_length):
            # input 为所有时刻单词的输入向量（N, T, D）
            input, _ = word_embedding_forward(start, W_embed)
            # print(input.shape)
            
            if self.cell_type == "rnn":
                next_h, _ = rnn_step_forward(input[:, 0, :], h0, Wx, Wh, b)
            elif self.cell_type == "lstm":
                # 下一时刻的细胞状态输入c直接为上一刻细胞输出
                next_h, c0, _ = lstm_step_forward(input[:, 0, :], h0, c0, Wx, Wh, b)
            else:
                raise ValueError('Invalid cell_type "%s"' % self.cell_type)
