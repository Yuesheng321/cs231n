...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = dnext_h*(1 - np.tanh(np.dot(prev_h, Wh) + np.dot(x, Wx) + b)**2)  # (N, H)
...
def rnn_backward(dh, cache):
    ...
    for t in range(T - 1, -1, -1):
        dx[:, t, :], dprev_h, d_Wx, d_Wh, d_b = rnn_step_backward(dh[:, t, :] + dprev_h, cache[t])
        dWx += d_Wx
        dWh += d_Wh
        db += d_b
    dh0 = dprev_h       
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # pass
    N, H = prev_c.shape
    A = np.dot(x, Wx) + np.dot(prev_h, Wh) + b  # shape(N, 4H)
    i, f, o, g = sigmoid(A[:, :H]), sigmoid(A[:, H:2*H]), sigmoid(A[:, 2*H:3*H]), np.tanh(A[:, 3*H:])
    next_c = f*prev_c + i*g
    next_h = o*np.tanh(next_c)
    cache = [x, prev_h, prev_c, Wx, Wh, b, next_c, g, i, o, f, A]
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    # pass
    x, prev_h, prev_c, Wx, Wh, b, next_c, g, i, o, f, A = cache
    N, H = dnext_h.shape
    # print(N, H)
    # shape(N, 4H)
    dA = np.zeros_like(A)
    ...
    # print(dA.shape, A.shape, di.shape)
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        # 1.每个时刻对应一个损失， 当前时刻h(t)的梯度由当前损失+h(t+1)刻传递过来的损失
        # 2.由于c(t)不对应损失函数，所以dc(t)为0
        dx[:, t, :], dprev_h, dprev_c, d_Wx, d_Wh, d_b = lstm_step_backward(dh[:, t, :]+dprev_h, dprev_c, cache[t])
        dWx += d_Wx
        dWh += d_Wh
        db += d_b
    dh0 = dprev_h
...
def temporal_softmax_loss(x, y, mask, verbose=False):
    ...
    As an additional complication混乱, we may want to ignore the model output at some
